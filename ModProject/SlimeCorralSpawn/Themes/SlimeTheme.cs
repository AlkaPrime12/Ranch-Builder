...
namespace SlimeCorralSpawn.Themes
{
    public static class SlimeTheme
    {
        ...
        /// <summary>
        /// SUPERFICIE de un icono/chip: en modo oscuro se mantiene CLARA a propósito.
        ///
        /// El bug que arreglaba: `Themed()` oscurece los pasteles, así que la cápsula del icono quedaba oscura…
        /// y el glifo de adentro ya se dibuja oscuro → negro sobre negro, invisible. En el estilo que queremos
        /// (fila oscura + círculo claro + glifo oscuro, como las filas de "Sandstone Wall"/"Marble Wall") la
        /// cápsula NO debe seguir al fondo: debe contrastar con él.
        /// </summary>
        public static Color IconSurface(Color accent)
        {
            Color soft = Color.Lerp(accent, Color.white, DarkMode ? 0.80f : 0.72f);
            soft.a = accent.a;
            return soft;   // deliberadamente SIN Themed(): tiene que quedar clara sobre el panel oscuro
        }

        /// <summary>Garantiza contraste mínimo de <paramref name="fg"/> contra <paramref name="bg"/>: si están
        /// demasiado cerca en luminancia, empuja el frente hacia blanco o negro (el que más contraste dé).</summary>
        public static Color EnsureContrast(Color bg, Color fg, float minDelta = 0.35f)
        {
            float lb = bg.r * 0.299f + bg.g * 0.587f + bg.b * 0.114f;
            float lf = fg.r * 0.299f + fg.g * 0.587f + fg.b * 0.114f;
            if (Mathf.Abs(lf - lb) >= minDelta) return fg;
            Color target = lb < 0.5f ? Color.white : Color.black;
            var r = Color.Lerp(fg, target, 0.65f);
            r.a = fg.a;
            return r;
        }
