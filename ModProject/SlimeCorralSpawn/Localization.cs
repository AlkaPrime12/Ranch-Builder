...
namespace SlimeCorralSpawn
{
    ...
    public static class Loc
    {
        ...
        private static readonly Dictionary<string, string[]> _t = new Dictionary<string, string[]>
        {
            ...
            { "cfg_dark_mode", new[] { "Modo oscuro", "Dark mode", "深色模式", "Тёмный режим", "Mode sombre" } },
            // Free Build: subtítulos de las tarjetas de herramientas + botón de borrar lo dibujado
            { "mat_picker_title", new[] { "MATERIAL", "MATERIAL", "材质", "МАТЕРИАЛ", "MATÉRIAU" } },
            { "cfg_ui_scale_tip", new[] { "Tamaño del menú: agranda o achica el panel y las letras.",
                                          "Menu size: makes the panel and text bigger or smaller.",
                                          "菜单大小：放大或缩小面板与文字。",
                                          "Размер меню: увеличить или уменьшить панель и текст.",
                                          "Taille du menu : agrandit ou réduit le panneau et le texte." } },
            { "fb_sub_floor",  new[] { "2 esquinas → rellena", "2 corners → fill", "两角 → 填充", "2 угла → заливка", "2 coins → remplir" } },
            { "fb_sub_draw",   new[] { "Pincel a mano alzada", "Freehand brush", "手绘笔刷", "Кисть от руки", "Pinceau à main levée" } },
            { "fb_sub_shape",  new[] { "Puntos → forma libre", "Points → free shape", "点 → 自由形状", "Точки → форма", "Points → forme libre" } },
            { "fb_sub_remove", new[] { "Apuntá y click (F9)", "Aim and click (F9)", "瞄准点击 (F9)", "Наведи и жми (F9)", "Visez et cliquez (F9)" } },
            { "fb_erase_drawn",new[] { "Borrar lo dibujado", "Erase drawn pieces", "清除已绘制", "Удалить нарисованное", "Effacer le dessin" } },
            { "fb_erase_sub",  new[] { "Suelos y formas a mano", "Hand-drawn floors/shapes", "手绘地板与形状", "Полы и формы от руки", "Sols et formes dessinés" } },
            { "fb_erase_tip",  new[] { "Borra TODO lo dibujado a mano (no toca las estructuras del catálogo).",
                                       "Erases ALL hand-drawn pieces (catalog structures are untouched).",
                                       "清除所有手绘部件（不影响目录结构）。",
                                       "Удаляет всё нарисованное вручную (каталожные постройки не трогает).",
                                       "Efface tout le dessin à la main (les structures du catalogue restent)." } },
            { "fb_erase_confirm", new[] { "¿Seguro? Click de nuevo para BORRAR.", "Sure? Click again to ERASE.",
                                          "确定吗？再次点击以清除。", "Уверены? Нажмите ещё раз, чтобы УДАЛИТЬ.",
                                          "Sûr ? Cliquez à nouveau pour EFFACER." } },
            ...
            { "scb_del_tool_btn", new[]{ "Borrar modelos de escena", "Delete scene models", "删除场景模型", "Удалить modelos", "Supprimer modèles" } },
            ...
            { "sbt_sel_hint_hover_del", new[]{
                "[Click] borrar · [Click der] salir",
                "[Click] delete · [Right click] exit",
                "[点击] 删除 · [右键] 退出",
                "[Клик] удалить · [ПКМ] выход",
                "[Clic] supprimer · [Clic droit] quitter" } },

            // ── Mega GUI del Scene Tool (panel de catálogo + barra inferior) ──
            { "st_title",   new[]{ "Modelos de Escena", "Scene Models", "场景模型", "Модели сцены", "Modèles de scène" } },
            { "st_fav",     new[]{ "Favoritos", "Favorites", "收藏", "Избранное", "Favoris" } },
            { "st_empty",   new[]{ "No hay modelos acá", "No models here", "这里没有模型", "Здесь нет моделей", "Aucun modèle ici" } },
            { "st_group_hdr", new[]{ "CATEGORÍA PRINCIPAL", "MAIN CATEGORY", "主分类", "ОСНОВНАЯ КАТЕГОРИЯ", "CATÉGORIE PRINCIPALE" } },

            // ── SlimeSpawner ──
            { "spw_btn",      new[]{ "SLIMESPAWNER", "SLIMESPAWNER", "SLIMESPAWNER", "SLIMESPAWNER", "SLIMESPAWNER" } },
            { "spw_title",    new[]{ "Spawners", "Spawners", "生成器", "Спавнеры", "Générateurs" } },
            { "spw_pick_hint",new[]{ "Elegí qué tipo de spawner querés colocar.", "Choose which spawner to place.", "选择要放置的生成器类型。", "Выберите тип спавнера.", "Choisissez le type de générateur." } },
            { "spw_slime",    new[]{ "Spawner de Slimes", "Slime Spawner", "史莱姆生成器", "Спавнер слаймов", "Générateur de slimes" } },
            { "spw_slime_desc",new[]{ "Genera slimes del juego cada cierto tiempo.", "Spawns the game's slimes periodically.", "定期生成游戏中的史莱姆。", "Периодически создаёт слаймов.", "Fait apparaître des slimes régulièrement." } },
            { "spw_hen",      new[]{ "Spawner de Gallinas", "Chicken Spawner", "小鸡生成器", "Спавнер куриц", "Générateur de poules" } },
            { "spw_hen_desc", new[]{ "Genera gallinas y pollitos del juego.", "Spawns the game's hens and chicks.", "生成游戏中的母鸡和小鸡。", "Создаёт куриц и цыплят.", "Fait apparaître poules et poussins." } },
            { "spw_freq",     new[]{ "Frecuencia de spawn", "Spawn frequency", "生成频率", "Частота спавна", "Fréquence" } },
            { "spw_max",      new[]{ "Máximo en la zona", "Max in the area", "区域上限", "Максимум в зоне", "Maximum dans la zone" } },
            { "spw_radius",   new[]{ "Radio de spawn", "Spawn radius", "生成半径", "Радиус спавна", "Rayon" } },
            { "spw_refill",   new[]{ "Si no queda ninguno, spawnear al toque", "If none are left, spawn immediately", "如果一个都不剩，立即生成", "Если никого не осталось — спавнить сразу", "S'il n'en reste aucun, générer aussitôt" } },
            { "spw_which",    new[]{ "Qué spawnea", "What it spawns", "生成内容", "Что создаёт", "Ce qu'il génère" } },
            { "spw_place",    new[]{ "Aceptar y colocar", "Accept and place", "确认并放置", "Принять и поставить", "Valider et placer" } },
            { "spw_pick_one", new[]{ "Elegí al menos uno", "Pick at least one", "至少选择一个", "Выберите хотя бы одного", "Choisissez-en au moins un" } },
            { "spw_back",     new[]{ "Volver", "Back", "返回", "Назад", "Retour" } },
            ...
            { "spw_radiant",  new[]{ "Modo radiante", "Radiant mode", "辐光模式", "Радиантный режим", "Mode radiant" } },
            { "cfg_harvest_hour", new[]{ "Hora de cosecha de los jardines", "Garden harvest hour", "花园收获时间", "Час сбора урожая", "Heure de récolte des jardins" } },
            { "cfg_diag",     new[]{ "Diagnósticos en el log (más lento al cargar)", "Diagnostics in the log (slower loading)", "日志诊断（加载更慢）", "Диагностика в логе (медленнее загрузка)", "Diagnostics dans le journal (chargement plus lent)" } },
            { "spw_visible",  new[]{ "Spawners visibles", "Show spawners", "显示生成器", "Показывать спавнеры", "Générateurs visibles" } },
            { "spw_largo",    new[]{ "Largo", "Largo", "巨型史莱姆", "Ларго", "Largo" } },
            { "spw_largo_with", new[]{ "Mezclado con", "Mixed with", "混合对象", "Смешан с", "Mélangé avec" } },
            { "spw_pick_partner", new[]{ "Elegí con qué slime se mezcla", "Pick which slime it mixes with", "选择与哪个史莱姆混合", "Выберите слайма для смешивания", "Choisissez le slime à mélanger" } },
            { "spw_yes",      new[]{ "sí", "yes", "是", "да", "oui" } },
            { "spw_no",       new[]{ "no", "no", "否", "нет", "non" } },
            { "spw_launch",   new[]{ "fuerza", "force", "力度", "сила", "force" } },
            { "spw_cursor_free", new[]{ "Cursor libre", "Cursor free", "光标自由", "Курсор свободен", "Curseur libre" } },
            { "spw_cursor_lock", new[]{ "Cursor fijo", "Cursor locked", "光标锁定", "Курсор закреплён", "Curseur verrouillé" } },
            { "spw_gz_move",  new[]{ "[1] Mover", "[1] Move", "[1] 移动", "[1] Двигать", "[1] Déplacer" } },
            { "spw_gz_rot",   new[]{ "[2] Rotar", "[2] Rotate", "[2] 旋转", "[2] Вращать", "[2] Pivoter" } },
            { "spw_hud1",     new[]{ "Click izquierdo: colocar  ·  Click derecho / Esc: cancelar",
                                     "Left click: place  ·  Right click / Esc: cancel",
                                     "左键：放置 · 右键/Esc：取消",
                                     "ЛКМ: поставить · ПКМ/Esc: отмена",
                                     "Clic gauche : placer · Clic droit / Échap : annuler" } },
            { "spw_hud_rot",  new[]{ "Arrastrá con el click para girar  ·  ENTER: colocar  ·  Click derecho / Esc: cancelar",
                                     "Drag with the click to turn  ·  ENTER: place  ·  Right click / Esc: cancel",
                                     "按住拖动以旋转 · ENTER：放置 · 右键/Esc：取消",
                                     "Тяните с зажатой ЛКМ, чтобы повернуть · ENTER: поставить · ПКМ/Esc: отмена",
                                     "Glissez en cliquant pour pivoter · ENTRÉE : placer · Clic droit / Échap : annuler" } },
            { "spw_hud2",     new[]{ "En [2] Rotar: rueda gira · Shift+rueda cambia la fuerza · Q/E giran suave",
                                     "In [2] Rotate: wheel turns · Shift+wheel changes force · Q/E turn slowly",
                                     "在[2]旋转中：滚轮转向 · Shift+滚轮改变力度 · Q/E 缓慢转动",
                                     "В [2] Вращать: колесо — поворот · Shift+колесо — сила · Q/E — плавно",
                                     "En [2] Pivoter : molette tourne · Maj+molette change la force · Q/E lentement" } },

            // ── Ctrl+Z ──
            { "undo_done",    new[]{ "Deshecho", "Undone", "已撤销", "Отменено", "Annulé" } },
            { "undo_empty",   new[]{ "Nada que deshacer", "Nothing to undo", "没有可撤销的操作", "Нечего отменять", "Rien à annuler" } },
            { "undo_place",   new[]{ "colocar", "place", "放置", "размещение", "placement" } },
            { "undo_delete",  new[]{ "borrar", "delete", "删除", "удаление", "suppression" } },
            { "undo_move",    new[]{ "mover", "move", "移动", "перемещение", "déplacement" } },
            { "undo_spawner", new[]{ "colocar spawner", "place spawner", "放置生成器", "размещение спавнера", "placer générateur" } },
            { "undo_draw",    new[]{ "dibujar", "draw", "绘制", "рисование", "dessin" } },
            { "undo_wipe",    new[]{ "borrar todos los dibujos", "erase all drawings", "清除所有绘制", "стереть все рисунки", "effacer tous les dessins" } },
            { "st_continuous", new[]{ "CONTINUO", "CONTINUOUS", "连续", "НЕПРЕРЫВНО", "CONTINU" } },
            { "st_delete",     new[]{ "BORRAR", "DELETE", "删除", "УДАЛИТЬ", "SUPPRIMER" } },
            { "st_freecam",    new[]{ "FREE CAM", "FREE CAM", "自由视角", "СВОБ. КАМЕРА", "CAM. LIBRE" } },
            { "st_cursor_locked", new[]{ "MIRA", "AIM", "瞄准", "ПРИЦЕЛ", "VISÉE" } },
            { "st_cursor_free",   new[]{ "CURSOR", "CURSOR", "光标", "КУРСОР", "CURSEUR" } },
            { "st_snap",       new[]{ "IMÁN", "SNAP", "吸附", "ПРИТЯЖ.", "AIMANT" } },
            { "st_exit_btn",   new[]{ "Salir", "Exit", "退出", "Выход", "Quitter" } },
            { "st_rmb",        new[]{ "Click der", "RMB", "右键", "ПКМ", "Clic droit" } },
            { "st_unavailable", new[]{ "Ese modelo todavia no esta guardado. Visita {0} una vez y se guarda solo.",
                                       "That model isn't saved yet. Visit {0} once and it saves itself.",
                                       "该模型尚未保存。前往 {0} 一次即可自动保存。",
                                       "Эта модель ещё не сохранена. Посетите {0} один раз — сохранится сама.",
                                       "Ce modèle n'est pas encore enregistré. Visitez {0} une fois, il s'enregistre seul." } },
            { "st_search_ph",  new[]{ "Buscar modelo…", "Search model…", "搜索模型…", "Поиск модели…", "Rechercher…" } },
            { "st_filter_all", new[]{ "Todos", "All", "全部", "Все", "Tous" } },
            { "st_recent",     new[]{ "Recientes", "Recent", "最近", "Недавние", "Récents" } },
            { "st_info_sel",   new[]{ "Seleccionado", "Selected", "已选", "Выбрано", "Sélectionné" } },
            { "st_info_line",  new[]{ "Escala {0} · {1} · Imán {2}", "Scale {0} · {1} · Snap {2}", "缩放 {0} · {1} · 吸附 {2}", "Масштаб {0} · {1} · Притяж. {2}", "Échelle {0} · {1} · Aimant {2}" } },
            { "st_hint_place", new[]{
                "[Click izq] colocar · [Click der] soltar · rueda escala · flechas altura · Q/Z girar",
                "[LMB] place · [RMB] drop · wheel scale · arrows height · Q/Z rotate",
                "[左键]放置 · [右键]取消 · 滚轮缩放 · 方向键高度 · Q/Z旋转",
                "[ЛКМ] ставить · [ПКМ] сбросить · колесо масштаб · стрелки высота · Q/Z поворот",
                "[Clic G] placer · [Clic D] lâcher · molette échelle · flèches hauteur · Q/Z pivoter" } },
            { "st_hint_browse", new[]{
                "Elegí un modelo (el cursor pasa a la mira) · WASD vuela · [E] borrar · [R] cursor · [Click der] salir",
                "Pick a model (cursor switches to aim) · WASD fly · [E] delete · [R] cursor · [RMB] exit",
                "选择模型(光标转为瞄准) · WASD飞行 · [E]删除 · [R]光标 · [右键]退出",
                "Выберите модель (курсор → прицел) · WASD полёт · [E] удалить · [R] курсор · [ПКМ] выход",
                "Choisissez un modèle (curseur → visée) · WASD voler · [E] supprimer · [R] curseur · [Clic D] sortir" } },

            // ── Nombres de las ZONAS REALES del juego (unifican las sub-zonas internas) ──
            { "zone_conservatory", new[]{ "El Conservatorio", "The Conservatory", "温室", "Оранжерея", "Le Conservatoire" } },
            { "zone_fields",       new[]{ "Campos Arcoíris", "Rainbow Fields", "彩虹田野", "Радужные поля", "Champs Arc-en-ciel" } },
            { "zone_ember",        new[]{ "Valle Ember", "Ember Valley", "余烬谷", "Тлеющая долина", "Vallée des Braises" } },
            { "zone_starlight",    new[]{ "Costa Estelar", "Starlight Strand", "星光海岸", "Звёздный берег", "Rivage Étoilé" } },
            { "zone_bluffs",       new[]{ "Acantilados Nevados", "Powderfall Bluffs", "落雪崖", "Снежные утёсы", "Falaises Enneigées" } },
            { "zone_labyrinth",    new[]{ "El Laberinto Gris", "The Grey Labyrinth", "灰色迷宫", "Серый лабиринт", "Le Labyrinthe Gris" } },
            { "zone_dreamland",    new[]{ "Tierra de Sueños", "Dream Land", "梦境之地", "Страна снов", "Pays des Rêves" } },
            { "zone_transitions",  new[]{ "Transiciones", "Transitions", "过渡区", "Переходы", "Transitions" } },
            ...
            // Subcategorías nuevas (catálogo en 2 niveles) + los 6 grupos grandes
            { "scbcat_Montanas",    new[]{ "Montañas", "Mountains", "山脉", "Горы", "Montagnes" } },
            { "scbcat_Acantilados", new[]{ "Acantilados", "Cliffs", "峭壁", "Утёсы", "Falaises" } },
            { "scbcat_Arena",       new[]{ "Arena", "Sand", "沙地", "Песок", "Sable" } },
            { "scbcat_Flores",      new[]{ "Flores", "Flowers", "花朵", "Цветы", "Fleurs" } },
            { "scbcat_Pasto",       new[]{ "Pasto", "Grass", "草地", "Трава", "Herbe" } },
            { "scbcat_Musgo",       new[]{ "Musgo", "Moss", "苔藓", "Мох", "Mousse" } },
            { "scbcat_Enredaderas", new[]{ "Enredaderas", "Vines", "藤蔓", "Лианы", "Lianes" } },
            { "scbcat_Coral",       new[]{ "Coral y algas", "Coral & kelp", "珊瑚与海藻", "Кораллы", "Corail & algues" } },
            { "scbcat_Arbustos",    new[]{ "Arbustos", "Bushes", "灌木", "Кусты", "Buissons" } },
            { "scbcat_Cristales",   new[]{ "Cristales", "Crystals", "水晶", "Кристаллы", "Cristaux" } },
            { "scbcat_Rocas grandes", new[]{ "Rocas grandes", "Boulders", "巨石", "Валуны", "Rochers" } },
            { "scbcat_Muros",       new[]{ "Muros", "Walls", "墙壁", "Стены", "Murs" } },
            { "scbcat_Puentes",     new[]{ "Puentes", "Bridges", "桥梁", "Мосты", "Ponts" } },
            { "scbcat_Puertas",     new[]{ "Puertas", "Doors", "门", "Двери", "Portes" } },
            { "scbcat_Escaleras",   new[]{ "Escaleras", "Stairs", "楼梯", "Лестницы", "Escaliers" } },
            { "scbcat_Plataformas", new[]{ "Plataformas", "Platforms", "平台", "Платформы", "Plateformes" } },
            { "scbcat_Techos",      new[]{ "Techos", "Roofs", "屋顶", "Крыши", "Toits" } },
            { "scbcat_Pilares",     new[]{ "Pilares", "Pillars", "柱子", "Колонны", "Piliers" } },
            { "scbcat_Edificios",   new[]{ "Edificios", "Buildings", "建筑", "Здания", "Bâtiments" } },
            { "scbcat_Tuberias",    new[]{ "Tuberías", "Pipes", "管道", "Трубы", "Tuyaux" } },
            { "scbcat_Estatuas",    new[]{ "Estatuas", "Statues", "雕像", "Статуи", "Statues" } },
            { "scbcat_Reliquias",   new[]{ "Reliquias", "Relics", "遗物", "Реликвии", "Reliques" } },
            { "scbgrp_Terreno",     new[]{ "Terreno", "Terrain", "地形", "Ландшафт", "Terrain" } },
            { "scbgrp_Vegetacion",  new[]{ "Vegetación", "Vegetation", "植被", "Растительность", "Végétation" } },
            { "scbgrp_Rocas",       new[]{ "Rocas", "Rocks", "岩石", "Камни", "Roches" } },
            { "scbgrp_Estructuras", new[]{ "Estructuras", "Structures", "建筑结构", "Постройки", "Structures" } },
            { "scbgrp_Ruinas",      new[]{ "Ruinas", "Ruins", "遗迹", "Руины", "Ruines" } },
            { "scbgrp_Decoracion",  new[]{ "Decoración", "Decoration", "装饰", "Декор", "Décoration" } },
